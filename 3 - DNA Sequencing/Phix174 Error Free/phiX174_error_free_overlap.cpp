...
//finding maximum overlap graph where every node represent a read and every node has only one outgoing edge that represents the maximum
...
void overLap_graph(const vector<string>& reads, int n, string& genome){
    ...
    while(visited.size() != n){
        ...
        // concatenating the current-next read with the genome sequence after removing the overlap from it.
        genome += reads[othread].substr(molap, READSIZE - molap);
        curRead = othread;
        visited.insert(curRead);  // start from this node now
    }

    //cut one of overlaps between the first and last reads of the genome
